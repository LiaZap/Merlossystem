...
const PRISMA_EXT = new Set([".prisma"]);

/**
 * MerlostoreChat: o ORM legado e Prisma e a migracao para Drizzle esta em
 * andamento (docs/adr/0002-orm-transicao-prisma-drizzle.md).
 *
 * Enquanto `true`, o uso de Prisma e a falta de auditoria em models .prisma
 * saem como AVISO. Se saissem como ERRO, `npm run compliance` reprovaria ~50
 * arquivos legados no dia 1 e o hook post-write-check bloquearia toda edicao.
 *
 * Tabelas NOVAS (Drizzle `pgTable`) continuam sendo ERRO quando falta auditoria.
 * Mudar para `false` junto com a flag de .claude/hooks/pre-write-guard.mjs.
 */
const MIGRACAO_ORM_EM_ANDAMENTO = true;
/** @type {'error'|'warn'} */
const NIVEL_LEGADO = MIGRACAO_ORM_EM_ANDAMENTO ? "warn" : "error";
...
/** @type {{id:string,level:'error'|'warn',re:RegExp,msg:string,ext?:Set<string>,skipIfLine?:RegExp}[]} */
const LINE_RULES = [
  {
    id: "prisma",
    level: NIVEL_LEGADO,
    re: /@prisma\/client|new\s+PrismaClient|from\s+["']prisma["']|require\(["']@prisma\/client["']\)/,
    msg: "Prisma detectado (legado). Codigo novo deve usar Drizzle — ver ADR 0002.",
  },
  {
    id: "sqlite",
    level: "error",
  ...
  // Codigo NOVO (Drizzle): delete fisico e sempre ERRO, sem excecao.
  {
    id: "delete-fisico",
    level: "error",
    re: /\bdb\.delete\s*\(|\bdrizzle[\w.]*\.delete\s*\(/,
    msg: "Delete fisico detectado. Use soft delete (is_deleted = true).",
  },
  // Codigo LEGADO (Prisma): mesmos 27 pontos herdados do MVP. Reportado como
  // aviso ate a migracao (ADR 0002) — vira erro quando a flag mudar.
  // Excecao legitima: exclusao LGPD (direito ao esquecimento) exige delete fisico;
  // marque a linha com `compliance:delete-fisico-lgpd` para isentar.
  {
    id: "delete-fisico-legado",
    level: NIVEL_LEGADO,
    re: /\bprisma\.\w+\.(delete|deleteMany)\s*\(|\.deleteMany\s*\(/,
    skipIfLine: /compliance:delete-fisico-lgpd/,
    msg: "Delete fisico (Prisma legado). Trocar por soft delete (is_deleted = true).",
  },
  {
    id: "drop-destrutivo",
    level: "error",
...
function walk(dir, acc) {
  ...
  for (const e of entries) {
    ...
    if (e.isDirectory()) {
      ...
      if (CODE_EXT.has(ext) || SQL_EXT.has(ext) || PRISMA_EXT.has(ext)) acc.push(full);
    }
  }
  return acc;
}
...
// ---------------------------------------------------------------------------
// Checagem de models Prisma (schema legado — mesma regra de auditoria)
// ---------------------------------------------------------------------------

/**
 * O ORM legado deste projeto e Prisma (ADR 0002), entao o auditor precisa
 * enxergar `prisma/schema.prisma` — senao a regra "toda tabela tem auditoria"
 * fica sem enforcement nenhum ate a migracao terminar.
 *
 * Prisma escreve em camelCase com `@map("snake_case")`; aceitamos as duas
 * grafias. `modified_by` tambem e exigido (quem alterou o registro).
 * Isencao: `compliance:append-only` na linha acima do `model` (ex.: ActivityLog).
 *
 * @param {string} content @param {string} file @param {Finding[]} findings
 */
function checkPrismaModels(content, file, findings) {
  const AUDIT_PRISMA = [
    ["created_at", /\bcreatedAt\b|created_at/],
    ["updated_at", /\bupdatedAt\b|updated_at/],
    ["deleted_at", /\bdeletedAt\b|deleted_at/],
    ["is_deleted", /\bisDeleted\b|is_deleted/],
    ["modified_by", /\bmodifiedBy\b|modified_by|\buserId\b|user_id/],
  ];
  const re = /^model\s+(\w+)\s*\{/gm;
  let m;
  while ((m = re.exec(content)) !== null) {
    if (content.slice(Math.max(0, m.index - 300), m.index).includes(APPEND_ONLY)) continue;
    const end = content.indexOf("\n}", m.index);
    const body = content.slice(m.index, end === -1 ? content.length : end);
    const missing = AUDIT_PRISMA.filter(([, r]) => !r.test(body)).map(([name]) => name);
    if (missing.length > 0) {
      findings.push({
        level: NIVEL_LEGADO,
        id: "model-sem-auditoria",
        file,
        line: content.slice(0, m.index).split("\n").length,
        msg: `Model "${m[1]}" sem colunas de auditoria: ${missing.join(", ")}.`,
      });
    }
  }
}
...
function checkSoftDeleteFilter(content, file, findings) {
  ...
  if (!hasSelect) return;
  if (content.includes("is_deleted")) return; // ja filtra em algum lugar
...
function analyzeFile(file) {
  ...
  lines.forEach((text, i) => {
    ...
    for (const rule of LINE_RULES) {
      ...
      if (rule.skipIfLine && rule.skipIfLine.test(text)) continue; // excecao marcada na linha
  ...
  if (PRISMA_EXT.has(ext)) checkPrismaModels(content, rel, findings);
...
function main() {
  ...
  if (fileFlag !== -1 && args[fileFlag + 1]) {
    ...
    files = walk(scanRoot, []);
    // O schema legado vive fora de src/. Sem isto, a regra de auditoria de
    // tabela nao cobre nenhum model do projeto.
    const prismaDir = join(ROOT, "prisma");
    if (scanRoot !== ROOT && existsSync(prismaDir)) files = files.concat(walk(prismaDir, []));
  }
