...
O MerlostoreChat usa **NextAuth**, nao o esquema de sessao-em-banco descrito
mais abaixo (que e o modelo-alvo da base).

| Item | Implementacao real |
|------|--------------------|
| Biblioteca | `next-auth` v4, provider Credentials |
| Config | `src/lib/auth.ts` (`authOptions`) |
| Handler | `src/app/api/auth/[...nextauth]/route.ts` |
| Sessao | **JWT** (`strategy: "jwt"`), com `id` e `role` no token |
| Senha | `bcryptjs`, comparacao em `authorize()`; hash com custo 12 no cadastro |
| Bloqueio | usuario com `isActive: false` nao autentica |
| Rastreio | `lastLoginAt` atualizado a cada login |
| Protecao de rota | `src/middleware.ts` — paginas **e** `/api/:path*` |
| Sem sessao | pagina redireciona para `/login?callbackUrl=...`; API responde `401` JSON |
| Excecoes | `src/lib/api-publica.ts` — 9 rotas sem sessao, cada uma com gate proprio |
| Segredos de webhook/cron | `src/lib/webhook-auth.ts` |
| Cadastro | `POST /api/register` — bootstrap do 1o admin; depois exige sessao de admin |

O middleware cobre `/api/:path*` inteiro, entao **rota nova nasce protegida**.
Liberar uma rota exige edita-la em `src/lib/api-publica.ts` — e o teste
`tests/api-publica.test.ts` falha se a lista publica mudar sem intencao.

Tabela completa de quem autentica como em [api.md](api.md#autenticacao).

## Fluxo de Autenticacao (modelo-alvo da base)
